...
import static com.gantang.tianshu.exchange.ExchangeCore.EVENTS_POOLING;
...
public final class OrderBookEventsHelper {
    ...
    public MatcherTradeEvent sendTradeEvent(final IOrder matchingOrder,
        ...
        log.debug("sendTradeEvent,matchingOrder:{}",matchingOrder);
        ...
        event.positionAction = matchingOrder.getPositionAction();
        return event;
...
//        event.size = order.getSize() - order.getFilled();
        ...
        event.positionAction = order.getPositionAction();
        return event;
    }
...
//        event.activeOrderSeq = cmd.seq;
        ...
        event.positionAction = cmd.getPositionAction();
    ...
    public MatcherTradeEvent createBinaryEventsChain(final long timestamp,
        ...
        for (int i = 0; i < dataArray.length; i += LONGS_PER_MESSAGE) {
        ...
        sections.forEach((section, events) -> {
            final long[] dataArray = events.stream()
                    .flatMap(evt -> Stream.of(
                            ...
                            evt.size,
                            evt.bidderHoldPrice,
                            (long)evt.positionAction.getCode()))
                    .mapToLong(s -> s)
                    .toArray();
