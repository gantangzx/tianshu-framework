...
import com.gantang.tianshu.exchange.common.*;
import com.gantang.tianshu.exchange.common.api.binary.BatchAddAccountsCommand;
import com.gantang.tianshu.exchange.common.api.binary.BatchAddSymbolsCommand;
import com.gantang.tianshu.exchange.common.api.binary.BinaryDataCommand;
import com.gantang.tianshu.exchange.common.api.reports.ReportQuery;
import com.gantang.tianshu.exchange.common.api.reports.ReportResult;
...
import com.gantang.tianshu.exchange.common.config.LoggingConfiguration;
import com.gantang.tianshu.exchange.common.config.OrdersProcessingConfiguration;
import com.gantang.tianshu.exchange.common.enums.*;
...
import com.gantang.tianshu.exchange.utils.CoreArithmeticUtils;
import com.gantang.tianshu.exchange.utils.SerializationUtils;
import com.gantang.tianshu.exchange.utils.UnsafeUtils;
...
public final class RiskEngine implements WriteBytesMarshallable {
    ...
    private CommandResultCode placeOrder(final OrderCommand cmd,
        ...
        if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIR) {
            ...
            log.debug("............canPlaceMarginOrder == Start...........");
            final boolean canPlaceOrder = canPlaceMarginOrder(cmd, userProfile, spec, position);
            log.debug("............canPlaceMarginOrder == End:{}...........",canPlaceOrder);
            if (canPlaceOrder) {
...
//            log.warn("orderAmount={} > userProfile.accounts.get({})={}", orderAmount, currency, userProfile.accounts.get(currency));
    ...
    private boolean canPlaceMarginOrder(final OrderCommand cmd,
        ...
        log.debug("===================>canPlaceMarginOrder::newRequiredMarginForSymbol:{}",newRequiredMarginForSymbol);
    ...
    public boolean handlerRiskRelease(final long seq, final OrderCommand cmd) {
        ...
        if (mte != null && mte.eventType != MatcherEventType.BINARY_EVENT) {
            ...
            // TODO processing order is reversed(处理颠倒顺序)
            if (spec.type == SymbolType.CURRENCY_EXCHANGE_PAIR) {
    ...
    private void handleMatcherRejectReduceEventExchange(final OrderCommand cmd,
        ...
        if (takerSell) {
            ...
            // TODO 订单类型。IOC_BUDGET - for REJECT应该在所有交易计算后释放剩余存款
