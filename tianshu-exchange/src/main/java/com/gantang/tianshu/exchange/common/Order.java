 ...
 */
package com.gantang.tianshu.exchange.common;
...
public final class Order implements WriteBytesMarshallable, IOrder {
    ...
    public Order(BytesIn<?> bytes) {
        ...
        this.positionAction = PositionAction.of(bytes.readByte());
    ...
    public void writeMarshallable(BytesOut bytes) {
        ...
        bytes.writeByte(positionAction.getCode());
        bytes.writeLong(uid);
    ...
    public String toString() {
        return "[" + orderId + " " + (action == OrderAction.ASK ? 'A' : 'B')
                + " " + (positionAction == PositionAction.OPEN ? 'O' : 'C')
                + price + ":" + size + "F" + filled
                + " U" + uid + "]";
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(orderId, action,positionAction, price, size, reserveBidPrice, filled,
                uid);
    }
