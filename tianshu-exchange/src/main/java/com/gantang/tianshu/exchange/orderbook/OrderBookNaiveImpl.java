 ...
 */
package com.gantang.tianshu.exchange.orderbook;
...
import com.gantang.tianshu.exchange.common.enums.PositionAction;
...
public final class OrderBookNaiveImpl implements IOrderBook {
    ...
    private void newOrderPlaceGtc(final OrderCommand cmd) {
        ...
        final PositionAction positionAction = cmd.positionAction;
     ...
     * 从orderId索引中删除完全匹配的订单
     * Fully matching orders are removed from orderId index
     * 如果发生任何交易—它们发送到tradesConsumer
     * Should any trades occur - they sent to tradesConsumer
     *
     * @param activeOrder     - GTC or IOC order to match，待撮合的限价或市价订单
     * @param matchingBuckets - sorted buckets map 排序的待撮合的限价订单桶映射
...
//        log.info("matchInstantly: {} {}", activeOrder, matchingBuckets);
